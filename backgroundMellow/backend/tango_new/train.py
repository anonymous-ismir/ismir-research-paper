...
import wandb
import torch
# MPS workaround: T5 encoder uses torch.abs(int64) which Apple MPS does not support
if torch.backends.mps.is_available():
    import transformers.models.t5.modeling_t5 as t5_modeling
    def _relative_position_bucket_mps(relative_position, bidirectional=True, num_buckets=32, max_distance=128):
        relative_buckets = 0
        if bidirectional:
            num_buckets //= 2
            relative_buckets += (relative_position > 0).to(torch.long) * num_buckets
            relative_position = torch.abs(relative_position.float()).long()
        else:
    ...
    t5_modeling.T5Attention._relative_position_bucket = staticmethod(_relative_position_bucket_mps)
...
def parse_args():
    ...
    parser.add_argument(
        "--scheduler_name", type=str, default="sd2-community/stable-diffusion-2-1",
