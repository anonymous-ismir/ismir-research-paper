...
def _ensure_audioldm_checkpoint(name):
    """Download AudioLDM checkpoint to cache if missing."""
    path = get_metadata()[name]["path"]
    if os.path.isfile(path):
        return path
    os.makedirs(os.path.dirname(path), exist_ok=True)
    from huggingface_hub import hf_hub_download
    # haoheliu/AudioLDM-S-Full has file "audioldm-s-full" (no .ckpt in repo)
    local_file = hf_hub_download(
        repo_id="haoheliu/AudioLDM-S-Full",
        filename="audioldm-s-full",
    )
...
def build_pretrained_models(name):
    ckpt_path = _ensure_audioldm_checkpoint(name)
