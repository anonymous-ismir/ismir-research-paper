# Disable wandb so training doesn't prompt for API key (old wandb doesn't support new long keys)
...
# --text_encoder_name="google/flan-t5-large" --scheduler_name="sd2-community/stable-diffusion-2-1" \
...
# --text_column captions --audio_column location --checkpointing_steps="best" --report_to none

# Continue training the LDM from our checkpoint using the --hf_model argument
accelerate launch train.py \
--train_file="data/train_mydataset.json"  \
